...
Для диагностики использовать doctor и recover inspect; recover apply закрывает run
или подтверждает исход attempt только по переданному пользователем доказательству.
Backup/restore всегда проверяют точный inventory manifest, SQLite и согласованность
ревизий JSON; restore требует точную фразу и сохраняет сырой quarantine-снимок
подтверждения. Privacy map и [карта данных](../../references/privacy.md) показывают
размещение PII, export создаёт проверяемую копию, redact и purge требуют точных фраз,
а purge ещё и обязательный backup_directory.
