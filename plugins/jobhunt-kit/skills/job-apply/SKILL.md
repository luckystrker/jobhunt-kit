...
2. `begin` создаёт единственное намерение и возвращает attempt_id и точный пакет.
   CLI уже сохраняет пакет в уникальный `packet_path` вида
   `D/materials/attempt-<attempt_id>.json`. Использовать именно возвращённый путь,
   не переименовывать и не собирать пакет вручную. Begin не отправляет отклик.
3. Только после успешного begin в этом процессе вызвать Hirify один раз. Чтобы не
   ...
   `node <P>/scripts/send-packet.mjs --data <D> <packet_path из begin>`.
